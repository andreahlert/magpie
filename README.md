...
> **As of PR 9 (issue [#1](https://github.com/andreahlert/magpie/issues/1)) the default flow is intent + lock (Model C).**
> See [`docs/adoption/`](docs/adoption/) for the full guide and [`projects/_example-airflow/`](projects/_example-airflow/) for a worked example.
>
> The legacy family-based `setup-steward` takeover remains available for adopters already on it. Steps 1 and 2 below describe that legacy path; the Model C quick start above is the new default.

### Quick start (Model C, default)

```bash
# 1. Bootstrap the framework snapshot (same recipes as the legacy
#    flow below). See docs/setup/install-recipes.md.

# 2. Author your intent. Start from the example.
cp .apache-steward/projects/_example-airflow/.apache-steward.intent.yaml \
...
Committed:
- `.apache-steward.intent.yaml` (source of truth, edited by hand).
- `.apache-steward.lock` (generated by `apply`; reviewers verify with `magpie plan`).
- `.apache-steward-overrides/` (rendered templates worth versioning).

Gitignored:
- `.apache-steward/` (framework snapshot).
- `.apache-steward.local.lock` (per-machine fetch record).
- the symlinks under `.claude/skills/` (regenerated by `apply`).
